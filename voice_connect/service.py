...
from .gateway.server import VoiceGateway, VoiceGatewayConfig
...
class VoiceConnectService:
    ...
    gateway: VoiceGateway = field(init=False)
    _thread: threading.Thread | None = field(init=False, default=None)
    _started: bool = field(init=False, default=False)

    def __post_init__(self) -> None:
        self.base_dir = Path(self.base_dir)
        self.gateway = VoiceGateway(self.base_dir, self.config)
    ...
    def start_background(self) -> None:
        ...
        self._thread = threading.Thread(target=_runner, name="VoiceConnectGateway", daemon=True)
        self._thread.start()
        self._started = True
    ...
    async def disconnect_device(self, target: str, *, reason: str = "Disconnected by Voice") -> dict[str, Any]:
...
_SERVICE: VoiceConnectService | None = None


def get_service(base_dir: str | Path) -> VoiceConnectService:
    global _SERVICE
    if _SERVICE is None:
        _SERVICE = VoiceConnectService(Path(base_dir))
    return _SERVICE
