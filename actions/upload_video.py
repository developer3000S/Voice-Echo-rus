...
in the new version") — this plugin expands it with the local language model
into a full, SEO-keyword-rich TikTok caption in the user's own language before
the automation starts, so no LLM latency lands in the middle of the click
sequence.
...
def _generate_caption(brief: str) -> str:
    ...
    with the local language model, in the same language as the brief,
    keyword-targeted at Tier-1 audiences (US/UK/CA/AU/DE...). Falls back to
    the brief itself if the call fails — the upload must never be blocked by
    a caption error."""
    try:
        prompt = (
...
def run(parameters: dict, player=None, session_memory=None) -> str:
    ...
    #    tool returns IMMEDIATELY so the assistant can speak right away; the actual
