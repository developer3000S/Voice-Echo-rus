...
def _speak_edge_native(text: str) -> None:
    ...
    audio_path = os.path.join(tempfile.gettempdir(), f"voice_edge_tts_{uuid.uuid4().hex}.mp3")
    try:
        # Use a male neural voice for app speech so daily briefing and alerts sound
        # closer to Voice's normal male audio output.
    ...
    player_alias = f"voice_tts_{uuid.uuid4().hex}"
    try:
        if os.name != "nt":
