...
Вкладка **«Панель»** показывает живые визуализации, которые настраиваются в **Настройки → Система и подключения**:

- **Визуализация системных метрик** (`dashboard_viz`) — осциллограмма загрузки CPU/RAM/сети, столбчатая диаграмма или радар подсистем
- **Визуализация голосового сообщения** (`voice_viz`) — анимация, которая оживает, пока Piper TTS озвучивает ответ. Варианты:
  - `Волна` — симметричная волновая развёртка (`wave`)
  - `Эквалайзер` — столбчатый анализатор (`bars`)
  - `Сфера` — пульсирующая сфера с кольцами эха (`orb`)
- **Индикатор голоса в чате** — сообщение ассистента, которое сейчас озвучивается, помечается мини-эквалайзером

Карточки «COMMAND» и «РЕЗУЛЬТАТ ДЕЙСТВИЯ» на панели показывают текст команды и ответа целиком с переносом строк, а не усечённую версию.

## Запуск приложений и редакторов

При запросе текстового редактора (Блокнот, gedit и др.) Voice Echo сначала проверяет его наличие в системе и при отсутствии открывает доступный аналог — предпочтительно **Visual Studio Code**. Замена применяется только к текстовым редакторам; для остальных приложений запрашиваемое имя сохраняется.

Тот же механизм используется при открытии сохранённых результатов (сводки YouTube, списки рейсов): файл открывается в Блокноте, если он установлен, иначе — в VS Code или другом доступном редакторе.
