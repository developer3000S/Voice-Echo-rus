"""
dashboard/server.py — Voice Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "Voice Uploads",
        Path.home() / "Documents" / "Voice Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'VOICE-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"Voice Dashboard Port {port}"
        prog_rule  = "Voice Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="voice_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until Voice is run as Administrator.")
...
def _ensure_ssl_certs() -> bool:
    ...
    key_path = certs / "voice.key"
    cert_path = certs / "voice.crt"
    ...
    try:
        ...
        subject = issuer = x509.Name([
            x509.NameAttribute(NameOID.COMMON_NAME, "Voice AI Local Remote"),
        ])
        alt_names = [
...
<p>Press <strong style="color:#dde3ed">Mobile Connect</strong> in Voice to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('voice_token','{tok}');
  sessionStorage.setItem('voice_key','{key}');
  localStorage.setItem('voice_device_token','{dev_tok}');
...
<p>Connecting to Voice…</p>
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "voice.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "voice.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Mobile Connect' in Voice UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
