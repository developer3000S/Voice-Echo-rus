...
class PluginManager:
    ...
    def register_voice(self, voice_obj) -> None:
        self.voice_echo = voice_obj
        for p in list(self.plugins):
            try:
                fn = getattr(p, "on_voice_created", None)
                if callable(fn):
                    fn(voice_obj)
            except Exception as exc:
                print(f"[Plugins] on_voice_created error: {exc}")
    ...
    def dispatch(self, hook: str, *args, **kwargs):
        ...
        for p in list(self.plugins):
            try:
                ...
                if callable(fn):
                    # call with voice echo if plugin expects it
                    try:
                        res = fn(*args, **kwargs, voice_echo=self.voice_echo)
                    except TypeError:
                        res = fn(*args, **kwargs)
                    if res is True:
                        return True
