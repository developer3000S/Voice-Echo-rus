...
import com.voice.connect.VoiceConnectForegroundService
import com.voice.connect.core.AgentStateStore
import com.voice.connect.core.ConnectionState
import com.voice.connect.core.DeviceCredential
import com.voice.connect.core.GatewayEndpoint
import com.voice.connect.core.PairingOffer
import com.voice.connect.network.VoiceGatewayDiscovery
import com.voice.connect.pairing.PairingPayloadParser
import com.voice.connect.pairing.PairingStorage
...
fun VoiceConnectApp(
    ...
    val discovery = remember { VoiceGatewayDiscovery(context) }
        ...
        NavHost(
                ...
                if (pairingOffer != null) {
                    ...
                    ManualConnectDialog(
                        ...
                        onConnect = {
                            ...
                            AgentStateStore.setGateway(GatewayEndpoint(name = "Voice PC", host = manualHost.trim(), port = p))
            ...
            composable("scanner") {
                QrScannerScreen(
                    ...
                    onScanned = { raw ->
                        ...
                        if (offer == null) {
                            scanError = "That QR code does not look like a Voice pairing code."
                            AgentStateStore.setError(scanError)
                        } else {
                            scanError = null
                            storage.saveGatewayHint(offer)
                            AgentStateStore.setPairingOffer(offer)
                            AgentStateStore.setGateway(GatewayEndpoint(name = "Voice PC", host = offer.host, port = offer.port))
            ...
            composable("home") {
                if (credential != null) {
                    ConnectedScreen(
                        ...
                        status = status,
                        onDisconnect = {
                            context.startService(Intent(context, VoiceConnectForegroundService::class.java).apply {
                                action = VoiceConnectForegroundService.ACTION_STOP
                            })
            ...
            composable("chat") {
                ChatScreen(
                    onBack = { navController.popBackStack() },
                    onSendMessage = { text ->
                        val intent = Intent(context, VoiceConnectForegroundService::class.java).apply {
                            action = VoiceConnectForegroundService.ACTION_SEND_CHAT
                            putExtra(VoiceConnectForegroundService.EXTRA_CHAT_TEXT, text)
                        }
...
fun WelcomeScreen(onNext: () -> Unit) {
    Column(modifier = Modifier.fillMaxSize(), verticalArrangement = Arrangement.Center, horizontalAlignment = Alignment.CenterHorizontally) {
        Text("VOICE", style = MaterialTheme.typography.displayLarge, fontWeight = FontWeight.Black, color = MaterialTheme.colorScheme.primary)
...
fun AboutScreen(onNext: () -> Unit) {
    Column(modifier = Modifier.fillMaxSize(), verticalArrangement = Arrangement.Center, horizontalAlignment = Alignment.CenterHorizontally) {
        GlassCard {
            ...
            Text("This agent acts as a secure physical bridge between your Android device and your PC's Voice AI.", color = androidx.compose.ui.graphics.Color.White, textAlign = androidx.compose.ui.text.style.TextAlign.Center)
            Spacer(modifier = Modifier.height(24.dp))
            Button(onClick = onNext, modifier = Modifier.fillMaxWidth()) {
                Text("Next")
            }
...
private fun StartupPermissionsScreen(
    ...
    Column(
        ...
        Text("Enable the permissions Voice Connect needs to control your phone reliably.", color = MaterialTheme.colorScheme.onSurfaceVariant)
...
private fun DiscoveryScreen(
    ...
    Column(
        ...
        Text("VOICE CONNECT", style = MaterialTheme.typography.headlineMedium, fontWeight = FontWeight.Black, color = androidx.compose.ui.graphics.Color.White)
        Spacer(Modifier.height(8.dp))
        Text("Connect this device to Voice AI.", color = MaterialTheme.colorScheme.onSurfaceVariant)
        ...
        Button(onClick = onFindVoice, modifier = Modifier.fillMaxWidth()) { Text("Find Voice") }
        Spacer(Modifier.height(12.dp))
...
private fun PairingPendingScreen(
    ...
    Column(Modifier.fillMaxSize().padding(24.dp), verticalArrangement = Arrangement.Center) {
        Text("CONNECT TO VOICE?", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Black)
        Spacer(Modifier.height(8.dp))
        Text("${gateway?.name ?: "Voice PC"}\n${gateway?.host ?: offer.host}:${offer.port}", color = MaterialTheme.colorScheme.onSurfaceVariant)
        Spacer(Modifier.height(20.dp))
        Card(colors = CardDefaults.cardColors(containerColor = androidx.compose.ui.graphics.Color.Black.copy(alpha = 0.55f), contentColor = androidx.compose.ui.graphics.Color.White), border = androidx.compose.foundation.BorderStroke(1.dp, MaterialTheme.colorScheme.primary.copy(alpha = 0.3f))) {
...
private fun ConnectedScreen(
    ...
    Column(
        ...
        Text("VOICE CONNECT", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Black, color = androidx.compose.ui.graphics.Color.White)
        Spacer(Modifier.height(8.dp))
        Text("● Connected", color = MaterialTheme.colorScheme.primary)
        Spacer(Modifier.height(20.dp))
        Card(colors = CardDefaults.cardColors(containerColor = androidx.compose.ui.graphics.Color.Black.copy(alpha = 0.55f), contentColor = androidx.compose.ui.graphics.Color.White), border = androidx.compose.foundation.BorderStroke(1.dp, MaterialTheme.colorScheme.primary.copy(alpha = 0.3f))) {
            Column(Modifier.padding(16.dp)) {
                Text(gateway?.name ?: "Voice PC", fontWeight = FontWeight.Bold)
        ...
        Card(
            ...
            Row(
                ...
                Column {
                    Text("Voice Chat", fontWeight = FontWeight.Bold, style = MaterialTheme.typography.titleMedium)
                    Spacer(Modifier.height(4.dp))
