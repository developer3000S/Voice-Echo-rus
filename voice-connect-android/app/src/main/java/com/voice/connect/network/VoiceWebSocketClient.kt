package com.voice.connect.network

import android.content.Context
import android.os.SystemClock
import com.voice.connect.commands.DeviceCommandHandler
import com.voice.connect.core.AgentStateStore
import com.voice.connect.core.VoiceProtocol
import com.voice.connect.core.ChatMessage
import com.voice.connect.core.ConnectionState
import com.voice.connect.core.DeviceCredential
import com.voice.connect.core.DeviceSnapshot
...
class VoiceWebSocketClient(
    ...
    private fun sendHello() {
        send(VoiceProtocol.hello(buildSnapshot()))
    ...
    private fun sendPairRequest() {
        ...
        send(VoiceProtocol.envelope(VoiceProtocol.PAIR_REQUEST, snapshot))
    ...
    private fun sendAuthenticate() {
        ...
        currentCredential = credential
        send(VoiceProtocol.authenticate(credential))
    ...
    private fun handleCommandMessage(root: JSONObject) {
        ...
        val response = VoiceProtocol.envelope(
            VoiceProtocol.RESULT,
            result.toJson(),
            requestId = requestId,
        )
        send(response)
    ...
    private inner class VoiceSocketListener : WebSocketListener() {
        ...
        override fun onMessage(webSocket: WebSocket, text: String) {
            runCatching {
                ...
                when (type) {
                    VoiceProtocol.PAIR_REQUEST -> {
                        AgentStateStore.setStatus("Waiting for approval")
                    }
                    VoiceProtocol.PAIR_APPROVED -> handlePairApproved(root)
                    VoiceProtocol.DEVICE_ONLINE -> {
                        AgentStateStore.setConnectionState(ConnectionState.CONNECTED)
                        AgentStateStore.setStatus("Connected")
                    }
                    VoiceProtocol.CAPABILITIES -> {
                        AgentStateStore.addLog("Capabilities synced")
                    }
                    VoiceProtocol.EXECUTE -> handleCommandMessage(root)
                    VoiceProtocol.PING -> {
                        send(VoiceProtocol.envelope(VoiceProtocol.PONG, JSONObject().put("status", "ok"), requestId = root.optString("request_id")))
                    }
                    VoiceProtocol.ERROR -> {
                        AgentStateStore.setError(root.optJSONObject("payload")?.optString("error"))
                    }
                    VoiceProtocol.CHAT_MESSAGE -> {
    ...
    fun sendChatMessage(text: String) {
        val payload = VoiceProtocol.chatMessage(text)
