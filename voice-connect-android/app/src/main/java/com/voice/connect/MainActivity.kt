...
import com.voice.connect.ui.VoiceConnectApp
import com.voice.connect.ui.theme.VoiceConnectTheme
...
class MainActivity : ComponentActivity() {
    ...
    private val notificationPermission = registerForActivityResult(ActivityResultContracts.RequestPermission()) { granted ->
        if (granted || Build.VERSION.SDK_INT < Build.VERSION_CODES.TIRAMISU) {
            ...
            pendingServiceStart = false
            AgentStateStore.setError("Notification permission is required for Voice Connect.")
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        storage.loadGatewayHint()?.let {
            ...
            AgentStateStore.setGateway(
                com.voice.connect.core.GatewayEndpoint(
                    name = "Voice PC",
                    host = it.host,
                    port = it.port,
                )
    ...
    private fun startGatewayService() {
        val endpoint = AgentStateStore.gateway.value
        if (endpoint != null || AgentStateStore.credential.value != null) {
            val intent = Intent(this, VoiceConnectForegroundService::class.java)
